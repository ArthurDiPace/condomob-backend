...
class ArquivoViewSet(LoggingMixin, viewsets.ModelViewSet):
    queryset = ArquivoHash.objects.all()
    ...
    def create(self, request):
        ...
        try:
            ...
            ArquivoHash.objects.create(
                hash=hashed_data,
                dados=json.loads(data_json),
                data_carregamento=timezone.now(),
                usuario=user
            )
    ...
    def segunda_via(self, request):
        ...
        try:
            arquivo_hash = ArquivoHash.objects.get(id=arquivo_id)
        
            df = pd.DataFrame(arquivo_hash.dados)
        ...
        except ArquivoHash.DoesNotExist:
            return Response({'error': 'ArquivoHash não encontrado'}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)